...
- **`playbooks/<name>/PLAYBOOK.md`** — read it, then follow it inline as next serial phase, carrying same args (`mode=`, `run-id=`, `run-ledger=`, `headless=`). Ship-root-relative.
- **`subagents/<name>.md`** — treat path as reviewer/checklist library despite directory name. Read file, apply checklist inline in current context, record findings, continue serially. Never delegate it.
- **`references/<file>`** — checklist/reference doc relative to current playbook. Exception: shared `references/risk-signals.md` plus `references/run-ledger.md` resolve at ship root.

Old skill/delegation-call syntax means: read corresponding bundled file, execute inline.
...
**Ambiguous prompts** — when two intents are equally plausible (e.g., "rebuild auth" could be EVOLVE or REMOVE+CREATE), ask exactly one disambiguating question in plain prose. Don't guess.
...
- **Never pause for user input.** Every question point auto-resolves with best judgement. **Record each assumption** ("Assumed: EVOLVE over CREATE because the route already exists") in the per-run ledger and surface them all in the Step 5 report.
...
| Mode | Gating |
|---|---|
| `production` | Ask "Proceed with this pipeline?" in plain prose before Step 4. Decline → stop. |
...
3. Read each triggered reviewer file under `subagents/*.md`, execute its checklist inline in current context, then record findings serially. Never delegate or parallelize.
...
**One core skill per /ship run.** Execute routed workflow plus triggered reviews serially in current context.
...
- **NEVER guess between two plausible intents**
  **Instead:** When prompt is genuinely ambiguous (CREATE vs EVOLVE, EVOLVE vs REMOVE+CREATE), ask exactly one plain-prose question. One question, then commit.
...
- **NEVER honor a `mode=fast` override on a high-risk change without surfacing the conflict**
  **Instead:** If `mode=fast` is requested for work that hits a risk signal (auth/payments/migrations/jobs/webhooks/destructive deletes/etc.), ask in plain prose: "Detected high-risk signals (e.g., payments). You requested fast mode — that skips the production gates. Confirm fast anyway, or upgrade to production?" Honor user choice.
