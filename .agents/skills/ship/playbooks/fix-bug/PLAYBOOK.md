...
**Mode safety override.** If `mode=fast` is requested for a bug touching any of the **risk signals** (canonical list in `ship`'s `references/risk-signals.md` — most relevant here: auth, payments, secrets, schema integrity, or external webhooks), surface the conflict in plain prose and confirm before honoring. Silent integration failures in those areas often have second-order causes the runtime contract surfaces — `mode=fast` skips that surfacing.
