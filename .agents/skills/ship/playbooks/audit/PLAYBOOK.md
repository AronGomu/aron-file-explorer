...
2. If the working tree is dirty, surface it: an audit on top of in-progress changes will tangle findings with WIP. Ask in plain prose: "(s)tash and audit clean tree / (a)udit current state / (q)uit". Default `s`.
...
## Phase 4 — Serial inline audits

Read each triggered reviewer checklist plus `simplify` and `harden-types`, then execute every audit inline, sequentially, in current context. Scope every audit to agreed Phase 1 scope, not only diff. Run each reviewer in report-only mode: consolidate findings in Phase 5, apply them under Phase 6 gate.

Apply failure/recording policy in `playbooks/add-feature/references/subagent-playbook.md`: classify each audit as mandatory or advisory, record valid output, then treat mandatory audit without valid output as blocked rather than clean.

No delegation fallback exists or is needed. Bundled playbooks plus reviewer files are read directly and executed serially.
...
- **NEVER honor `mode=fast` on a request that explicitly says "deep dive" or "production"**
  **Instead:** surface the conflict in plain prose and let the user pick informed.
