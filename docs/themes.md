...
4. E4. Save as `*.theme.json`, restart app. This slice discovers files at startup; file watching is not implemented yet.
...
Invalid/missing selected files use OS-matched embedded palette at cold start without repairing files or changing saved ID. Runtime editing/hot-reload retention belongs to subsequent watcher work. Valid custom colors may have low contrast: schema validity is not an accessibility guarantee. Bundled AA token pairs are tested; broader rendered-component migration remains separate.
...
Diagnostics use existing error toasts for 5000ms; basenames/IDs/reasons only, never JSON bodies. Identical outstanding startup issues deduplicated across rerenders/selections.
